...
class MobileDetection(Controller):
    def on_filter_button_click(self,filter_object):
        ...
        start_time = filter_object.widgets['start_datetime'].dateTime().toString(Qt.ISODate)
        end_time = filter_object.widgets['end_datetime'].dateTime().toString(Qt.ISODate)
        self.data = list(self.model.get_mobile_usages(site_name,floor_name,start_time,end_time))
    ...
    def display_bboxes(self,display_object):
        ...
        for document in current_data['documents']:
            for bbox in document['img_data']['phone_results']:
                bbox = bbox['bbox']
                x1,y1,x2,y2 = bbox
                width,height = self.current_image_size.width(),self.current_image_size.height()
                x1 = x1 * width
                y1 = y1 * height
                x2 = x2 * width
                y2 = y2 * height
